...
def allocate(intervals, withheld_spans=None, *, current_day=None, deferred_spans=None):
    used = defaultdict(int)
    segments = []
    withheld_spans = [] if withheld_spans is None else withheld_spans
    deferred_spans = [] if deferred_spans is None else deferred_spans
    ...
    for interval in sorted(intervals, key=lambda i: i['start_at'] or ''):
        interval.update(expected_minutes=None, allocated_minutes=0, withheld_minutes=None, deferred_minutes=None, submission_eligible=False)
        if not interval['start_at'] or not interval['end_at'] or interval['end_at'] <= interval['start_at']:
            continue
        interval.update(expected_minutes=0, withheld_minutes=0, deferred_minutes=0)
        for start, end in split_midnights(datetime.fromisoformat(interval['start_at']), datetime.fromisoformat(interval['end_at'])):
            ...
            if current_day and day.isoformat() == current_day:
                interval['deferred_minutes'] += duration
                deferred_spans.append(dict(parent_interval_id=interval['interval_id'], local_date=day.isoformat(),
                    start_at=start.isoformat(), end_at=end.isoformat(), duration_minutes=duration,
                    status='deferred', reasons=['current_day_not_finished'], submission_eligible=False))
                continue
            ...
            interval['submission_eligible'] = True
            ...
            for category, minutes in [('regular_remote',regular),('overtime_remote',duration-regular)]:
                ...
                segments.append(dict(segment_id=f"{interval['interval_id']}:segment:{len(segments)}",
                    ...
                    policy_version=POLICY_VERSION, submission_eligible=True))
                start = stop
    return segments
