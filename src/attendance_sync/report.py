...
def build_report(posts, own_id, start, end):
    ...
    all_withheld = []
    all_segments = allocate(intervals, all_withheld)
    ...
    report = dict(schema_version=2, window={'from':start.isoformat(),'to':end.isoformat(),'timezone':'Asia/Tehran'},
...
def render_html(report):
    ...
    content.append('<h2>Daily summary</h2><p>Allocated and withheld selected minutes are separate; unresolved durations are unknown. Separate spans preserve gaps.</p><table><tr><th>Date / Jalali</th><th>Work spans</th><th>Regular min</th><th>Overtime min</th><th>Total min</th><th>Withheld min</th><th>Review reasons</th></tr>')
    for day, row in sorted(days.items()):
        ...
        values = [day + ' / ' + jalali, ', '.join(row['times']) or 'Unresolved', row['regular'], row['overtime'], row['regular'] + row['overtime'], row.get('withheld', 0), ', '.join(sorted(row['reasons'])) or 'None']
    ...
    content.append(f'<tr><th>Total</th><td></td><td>{regular}</td><td>{overtime}</td><td>{regular + overtime}</td><td>{withheld}</td><td></td></tr></table>')
    ...
    for section in ('events','context_events','ranges','context_ranges','intervals','context_intervals','segments','allocation_blockers','withheld_spans','context_withheld_spans','ignored','labels'):
        rows = report[section]
